...
def one_and_two_digit(number)
  if number < 21
    number_word_string = WORD_MAP[number]
  else
    number_word_string = ""
    number_word_string.concat( WORD_MAP[(number / 10 * 10)] )
    number_word_string.concat( WORD_MAP[(number % 10)] )
  end
...
def three_digit(number)
  if number % 100 == 0
    number_word_string = WORD_MAP[(number / 100)] + " hundred"
  else
    number_word_string = WORD_MAP[(number / 100)] + " hundred and " + one_and_two_digit(number % 100)
  end
  return number_word_string
end
...
def four_digit(number)
  number_word_string = ""
  number_word_string.concat( WORD_MAP[(number / 1000)] + " thousand")
  return number_word_string
end
...
# => "All the letter added up are 21124"
